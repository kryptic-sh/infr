...
extern "C" {
    ...
    fn hipModuleGetFunction(func: *mut hipFunction_t, module: hipModule_t, name: *const c_char) -> c_int;
    fn hipModuleLaunchKernel(
        f: hipFunction_t, gx: u32, gy: u32, gz: u32, bx: u32, by: u32, bz: u32,
        shm: u32, stream: *mut c_void, kp: *mut *mut c_void, extra: *mut *mut c_void,
    ) -> c_int;
}
#[link(name = "hiprtc")]
extern "C" {
    fn hiprtcCreateProgram(p: *mut hiprtcProgram, src: *const c_char, name: *const c_char,
        nh: c_int, hdrs: *const *const c_char, incl: *const *const c_char) -> c_int;
...
fn main() {
    ...
    check(unsafe { hipGetDeviceCount(&mut count) }, "hipGetDeviceCount");
...
"#;
    ...
    check(unsafe {
        hiprtcCreateProgram(&mut prog, csrc.as_ptr(), cname.as_ptr(), 0, std::ptr::null(), std::ptr::null())
    }, "hiprtcCreateProgram");

    let std_flag = CString::new("-std=c++17").unwrap();
    let opts: [*const c_char; 1] = [std_flag.as_ptr()];
    check(unsafe { hiprtcCompileProgram(prog, 1, opts.as_ptr()) }, "hiprtcCompileProgram");

    let mut code_size: usize = 0;
    check(unsafe { hiprtcGetCodeSize(prog, &mut code_size) }, "hiprtcGetCodeSize");
    ...
    check(unsafe { hipModuleLoadData(&mut module, code.as_ptr() as *const c_void) }, "hipModuleLoadData");
    let cfn = CString::new("add_one").unwrap();
    let mut func: hipFunction_t = std::ptr::null_mut();
    check(unsafe { hipModuleGetFunction(&mut func, module, cfn.as_ptr()) }, "hipModuleGetFunction");

    // 4. Alloc GPU buffer, upload data, launch kernel, download
    let n: i32 = 1024;
    let mut dptr: *mut c_void = std::ptr::null_mut();
    check(unsafe { hipMalloc(&mut dptr, (n as usize) * 4) }, "hipMalloc");
    let host: Vec<f32> = (0..n).map(|i| i as f32).collect();
    check(unsafe { hipMemcpy(dptr, host.as_ptr() as *const c_void, host.len() * 4, H2D) }, "hipMemcpy H2D");
    ...
    check(unsafe {
        hipModuleLaunchKernel(func, grid_size, 1, 1, block_size, 1, 1, 0, stream, args.as_mut_ptr(), std::ptr::null_mut())
    }, "hipModuleLaunchKernel");

    check(unsafe { hipStreamSynchronize(stream) }, "hipStreamSynchronize");

    let mut result: Vec<f32> = vec![0.0; n as usize];
    check(unsafe { hipMemcpy(result.as_mut_ptr() as *mut c_void, dptr, result.len() * 4, D2H) }, "hipMemcpy D2H");
