...
// ---------------------------------------------------------------------------
// Chat template rendering (minijinja)
// ---------------------------------------------------------------------------

/// Render a jinja2 chat template (as embedded in a GGUF file) to a prompt string.
///
/// `template_jinja` — the raw jinja source from GGUF metadata.
/// `messages`       — normalised conversation turns.
/// `tools_json`     — optional JSON array of OpenAI-style tool definitions.
///
/// # Limitations
/// DiffusionGemma's embedded template uses Jinja2 macros, `namespace()`, and
/// custom filters not present in minijinja's default environment. The
/// implementation below adds the most common missing pieces (`raise_exception`,
/// `tojson`, `joiner`) and attempts a full render. If that fails, a best-effort
/// manual render is returned so callers always get a usable prompt.
///
/// TODO(sonnet): full template fidelity — minijinja lacks `namespace()` write-back
/// (the `ns.xxx = …` pattern), Jinja2 `set` with `namespace`, and some custom
/// filters the DiffusionGemma template uses. A complete solution requires either
/// a fork of minijinja that implements mutable namespace objects, or pre-processing
/// the template to rewrite namespace assignments into a form minijinja handles.
pub fn apply_chat_template(
    template_jinja: &str,
    messages: &[ChatMessage],
    tools_json: Option<&Value>,
) -> infr_core::error::Result<String> {
    use minijinja::{context, Environment, Value as JValue};

    let mut env = Environment::new();

    // Register helpers the DiffusionGemma template expects.
    env.add_function(
        "raise_exception",
    ...
    // `tojson` filter — serialize a value to a JSON string.
    env.add_filter("tojson", |val: JValue| -> String {
        serde_json::to_string(&val).unwrap_or_else(|_| "null".to_owned())
    });

    env.add_template("chat", template_jinja)
        .map_err(|e| infr_core::error::Error::Other(format!("template parse error: {e}")))?;

    let tmpl = env.get_template("chat").unwrap();

    // Convert messages to minijinja-friendly form.
    let msgs_json: Vec<Value> = messages
        .iter()
        .map(|m| {
            let mut obj = serde_json::Map::new();
            obj.insert("role".to_owned(), Value::String(m.role.clone()));
            obj.insert("content".to_owned(), Value::String(m.content.clone()));
            if let Some(id) = &m.tool_call_id {
                obj.insert("tool_call_id".to_owned(), Value::String(id.clone()));
            }
    ...
    let tools_val = tools_json.cloned().unwrap_or(Value::Null);
    ...
    match tmpl.render(ctx) {
        ...
        Err(e) => {
            tracing::warn!("minijinja template render failed ({e}); falling back to manual render");
            Ok(manual_render(messages))
        }
    }
}
...
fn manual_render(messages: &[ChatMessage]) -> String {
    ...
    // Opening tag for the assistant turn (generation prompt).
