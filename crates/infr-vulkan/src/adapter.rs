...
fn lower_op(
    ...
    match op {
        ...
        Op::MoeFfn {
            ...
            // kernel table, so there's no per-role dtype restriction beyond that shared set. Other
            // codebook quants (IQ*) still have no dp4a-mmq kernel and route through the per-token
