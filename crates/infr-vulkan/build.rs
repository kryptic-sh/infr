...
fn main() {
    ...
    // Generated resident-name → `-DSTREAMED` twin SPIR-V lookup for the dense prefill GEMM family
    // (native_gemm / native_gemm_warp). `Recorder::matmul_native_{off,f16a,splitk}` compute the
    // resident kernel name for the tile the resident path would pick, then (when a streamed dense
    // Linear) remap it here to the arena-addressed twin — keeping the tile selection single-sourced
    // in the recorder while every twin the build emitted is covered by construction (no hand-kept
    // parallel match to drift). Keyed by resident dst_stem; native_gemv keeps its own selector.
    let mut gemm_twins: Vec<String> = builds
        .iter()
        .filter_map(|(_, dst, _)| {
            dst.strip_suffix("_streamed")
                .filter(|r| r.starts_with("native_gemm"))
                .map(|r| r.to_string())
        })
        .collect();
    gemm_twins.sort();
    ...
    let mut map = String::from(
        "// Generated by build.rs — resident dense-GEMM kernel name to its -DSTREAMED twin\n\
         // (kernel-cache name + SPIR-V). Both are 'static literals so the twin caches distinctly.\n\
         pub(crate) fn native_gemm_streamed_spv(name: &str) -> (&'static str, &'static [u32]) {\n\
         \x20   macro_rules! v {\n\
         \x20       ($n:literal) => {{\n\
    ...
    map += "        _ => panic!(\"no -DSTREAMED twin for dense GEMM kernel {name}\"),\n    }\n}\n";
    std::fs::write(format!("{out}/native_streamed_gemm_map.rs"), map)
        .expect("write native_streamed_gemm_map.rs");
}
