...
impl<'a> Recorder<'a> {
    ...
    /// Split-K prefill attention: like `attention_prefill` but partitions the kv loop across
    /// `n_splits` workgroups per (query-tile, head) — for occupancy at small chunks (few query
    /// tiles). Pass 1 writes unnormalized partials (`ppm`/`ppl`/`ppacc`, sized
    /// `n_qtiles*nh*n_splits` × {BM, BM, BM*hd}); pass 2 combines them into `o`. `split_len` =
    /// kv keys per split (multiple of 64). `hd` multiple of 16, <= 128.
    #[allow(clippy::too_many_arguments)]
    pub fn attention_prefill_split(
        &self,
        q: &dyn Buffer,
        kc: &dyn Buffer,
        vc: &dyn Buffer,
        ...
        let n_qtiles = q_len.div_ceil(64);
        // pass 1: per-(qtile, head, split) partials
        self.stamp("attn_pf_part");
        let kp = self.be.kernel_spv_sg(
            "attn_prefill_partial",
            crate::gemm::attn_prefill_partial_spv(),
            6,
            32,
            32,
        );
        let mut p1 = [0u8; 32];
        p1[0..4].copy_from_slice(&(q_len as u32).to_ne_bytes());
        p1[4..8].copy_from_slice(&(kv_len as u32).to_ne_bytes());
        p1[8..12].copy_from_slice(&(nh as u32).to_ne_bytes());
        p1[12..16].copy_from_slice(&(nkv as u32).to_ne_bytes());
        p1[16..20].copy_from_slice(&(hd as u32).to_ne_bytes());
        p1[20..24].copy_from_slice(&(pos_offset as u32).to_ne_bytes());
        p1[24..28].copy_from_slice(&(n_splits as u32).to_ne_bytes());
        p1[28..32].copy_from_slice(&(split_len as u32).to_ne_bytes());
        self.dispatch(
            kp,
            &[
                Self::vkb(q),
            ...
            3,
            &p1,
            (n_qtiles * nh * n_splits) as u32,
        );
        // pass 2: combine splits → o
        self.stamp("attn_pf_comb");
        let kc2 = self.be.kernel_spv(
            "attn_prefill_combine",
            crate::gemm::attn_prefill_combine_spv(),
            4,
            12,
        );
        let mut p2 = [0u8; 12];
        ...
        p2[8..12].copy_from_slice(&(n_splits as u32).to_ne_bytes());
        self.dispatch(
            kc2,
            &[
                Self::vkb(ppm),
...
mod tests {
    ...
    fn run_attn_prefill_split(
        ...
        let bv = upf16(&be, &vp);
        let bo = be.alloc(mpad * nh * hd * 4, BufferUsage::Readback).unwrap();
        let ppm = be.alloc(nparts * 64 * 4, BufferUsage::Activations).unwrap();
        let ppl = be.alloc(nparts * 64 * 4, BufferUsage::Activations).unwrap();
        let ppacc = be
            .alloc(nparts * 64 * hd * 4, BufferUsage::Activations)
            .unwrap();
        let rec = be.recorder().unwrap();
        ...
        let err = got[..q_len * nh * hd]
            ...
            .fold(0f32, f32::max);
        println!(
            "attn_prefill_split q_len={q_len} kv_len={kv_len} splits={n_splits} max_err={err:e}"
        );
        assert!(err < 5e-3, "attn_prefill_split mismatch: {err}");
    }
    ...
    #[test]
    #[ignore = "requires a Vulkan GPU"]
    fn attention_prefill_split_matches_cpu() {
        run_attn_prefill_split(64, 512, 2, 1, 128, 4); // 1 qtile, 4 splits
        run_attn_prefill_split(64, 500, 4, 2, 128, 8); // partial last split, GQA, splits>kv-tiles edge
        run_attn_prefill_split(128, 2000, 2, 1, 128, 8); // 2 qtiles, causal across splits
        run_attn_prefill_split(70, 300, 9, 3, 64, 4); // q_len not %64, hd=64, GQA 3:1
    }
